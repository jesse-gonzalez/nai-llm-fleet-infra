...
# echo "INFO - Validating apps"
# find ./apps -maxdepth 4 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
#   do
...
# echo "INFO - Validating platform services"
# find ./platforms -maxdepth 4 -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
#   do
#     kubeconform "${kubeconform_flags[@]}" "${kubeconform_config[@]}" "${file}"
#     if [[ ${PIPESTATUS[0]} != 0 ]]; then
#       exit 1
